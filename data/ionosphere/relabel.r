...
library(MASS)
train.lda <- lda(as.formula(outcome~.), data = train)
...
c1 = train[which(train$outcome == 1),][which(class == 0),]
c2 = train[which(train$outcome == 1),][which(class == 1),]
c3 = train[which(train$outcome == 1),][which(class == 2),]
non_default = train[which(train$outcome == 0),]
relabel = rbind(non_default,c1,c2,c3)
relabel$outcome = c(rep('c1',nrow(non_default)),rep('c2',nrow(c1)),rep('c3',nrow(c2)),rep('c4',nrow(c3)))

train.qda <- lda(as.formula(outcome~.), data = relabel)
...
library(mixtools)
default = train[train$outcome == 1,]
gm<-mvnormalmixEM(default[,-33], k=2)
label = apply(gm$posterior, 1, which.max)
table(label)
